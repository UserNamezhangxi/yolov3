...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = 'map_out'):
    GT_PATH = os.path.join(os.getcwd(), path, 'ground-truth')
    DR_PATH = os.path.join(os.getcwd(), path, 'detection-results')
    # if there are no images then no animation can be shown
    IMG_PATH = os.path.join(os.getcwd(), path, 'images-optional')
    TEMP_FILES_PATH = os.path.join(os.getcwd(), path, '.temp_files')
    OUTPUT_FILES_PATH = os.path.join(os.getcwd(), path, 'result')
    ...
    if draw_plot:
        try:
            ...
            pass
        os.makedirs(os.path.join(OUTPUT_FILES_PATH, "F1"))
        os.makedirs(os.path.join(OUTPUT_FILES_PATH, "Recall"))
        os.makedirs(os.path.join(OUTPUT_FILES_PATH, "Precision"))
        os.makedirs(os.path.join(OUTPUT_FILES_PATH, "classesAP"))
    if show_animation:
    ...
    with open(OUTPUT_FILES_PATH + '/result.txt', 'w') as output_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if draw_plot:
                ...
                #plt.show()
                # save the plot
                fig.savefig(OUTPUT_FILES_PATH + "/classesAP/" + class_name + ".png")
                ...
                fig.savefig(OUTPUT_FILES_PATH + "/F1/" + class_name + ".png")
                plt.cla()
                ...
                axes.set_xlim([0.0, 1.0])
                axes.set_ylim([0.0, 1.05])
                fig.savefig(OUTPUT_FILES_PATH + "/Recall/" + class_name + ".png")
                plt.cla()
                ...
                axes.set_xlim([0.0, 1.0])
                axes.set_ylim([0.0, 1.05])
                fig.savefig(OUTPUT_FILES_PATH + "/Precision/" + class_name + ".png")
                plt.cla()
